...
export const sendOtpEmail = async (email: string, otp: string, name?: string) => {
     try {
          const info = await transporter.sendMail({
               from: "PEMIRA" + process.env.SMTP_FROM || '"PEMIRA STTNF" <no-reply@pemira.sttnf.ac.id>',
               to: email,
               ...
               html: `
                ...
                <body style="margin: 0; padding: 0; background-color: #f6f9fc; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 480px; margin: 40px auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
                        ...
                        <tr>
                            <td style="background: linear-gradient(135deg, #0248a3 0%, #00337d 100%); padding: 40px 20px; text-align: center;">
                                <div style="display: inline-block; background-color: white; padding: 12px; border-radius: 12px; margin-bottom: 16px;">
                                    ...
                                    <img src="https://pemira.sttnf.ac.id/pemira-logo.png" alt="PEMIRA Logo" style="width: 48px; height: 48px; object-fit: contain; display: block;" onerror="this.style.display='none'">
                                </div>
